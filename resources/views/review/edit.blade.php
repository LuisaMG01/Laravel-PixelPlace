...
@section('content')
    <div class="max-w-md mx-auto bg-white rounded-lg overflow-hidden shadow-md">
        <div class="px-6 py-4">
            <h2 class="text-2xl font-semibold text-gray-800 mb-2">{{ __('app.update_review') }}</h2>
            <form method="POST" action="{{ route('reviews.update', ['id' => $viewData['review']->getId()]) }}">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="description" class="block text-gray-700 text-sm font-bold mb-2">{{ __('app.description_edit') }}:</label>
                ...
                <div class="mb-4">
                    <label for="rating" class="block text-gray-700 text-sm font-bold mb-2">{{ __('app.rating_edit') }}:</label>
